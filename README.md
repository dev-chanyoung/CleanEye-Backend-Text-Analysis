...
3. **API 재호출 방지(Caching):** DB는 한 번 등록되면 영구히 유지되는 신뢰 소스이고, **Caffeine Cache**는 그 앞이 아니라 뒤에서 "아직 DB에 없는 단어"에 대해서만 짧은 시간(TTL) 안에 같은 단어가 반복 요청될 때 Gemini API를 다시 호출하지 않도록 막는 계층입니다. 이 캐시가 적중하면 **131ms**대의 응답 속도를 유지합니다.
